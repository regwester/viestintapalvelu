angular.module('report').directive('limitedParagraph', function() {
  ...
  return {
    restrict: 'E',
    replace: true,
    template: '<p class="limited-paragreph">{{ content | limitTo : getLimit() }}<span ng-if="isLimited"><span ng-if="!showAll">... </span><a ng-click="toggleShowAll()">{{ getToggleButtonText() }}</a><span></p>',
    scope: {
      'content': '=',
      'showButtonText': '@',
      'hideButtonText': '@',
