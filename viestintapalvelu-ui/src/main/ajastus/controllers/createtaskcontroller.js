ajastusApp.factory('CreateScheduledTask', function ($resource) {
...
ajastusApp.controller('CreateTaskController', ['$scope', '$location',
            '$filter', 'CreateScheduledTask', 'Hakus', 'Tasks', 'HakuNameByLocale',
    function ($scope, $location, $filter, CreateScheduledTask, Hakus, Tasks, HakuNameByLocale) {
        ...
        $scope.selectedDate = $filter('date')(new Date(), 'd.M.yyyy');
        ...
        $scope.create = function () {
            ...
            CreateScheduledTask.post({}, task, function () {
                $scope.back();
            });
        };
