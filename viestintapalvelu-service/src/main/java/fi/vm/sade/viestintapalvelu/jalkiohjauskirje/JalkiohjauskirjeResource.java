...
public class JalkiohjauskirjeResource extends AsynchronousResource {
    ...
    private final static String ApiPDFSync = "Palauttaa URLin, josta voi ladata jälkiohjauskirjeen/kirjeet PDF-muodossa; synkroninen. " + FixedTemplateNote; 
    private final static String ApiZIPSync = "Palauttaa URLin, josta voi ladata jälkiohjauskirjeen/kirjeet Itellan ZIP-muodossa; synkroninen. " + FixedTemplateNote; 
    ...
    public Response pdf(@ApiParam(value = "Muodostettavien jälkiohjauskirjeiden tiedot (1-n)", required = true) JalkiohjauskirjeBatch jalkiohjauskirjeBatch, @Context HttpServletRequest request)
    ...
    public Response zip(@ApiParam(value = "Muodostettavien jälkiohjauskirjeiden tiedot (1-n)", required = true) JalkiohjauskirjeBatch input, @Context HttpServletRequest request) throws IOException,
            DocumentException, NoSuchAlgorithmException {
    	String documentId;
    	try {
    		byte[] zip = jalkiohjauskirjeBuilder.printZIP(input);
    		documentId = downloadCache.addDocument(new Download("application/zip", "jalkiohjauskirje.zip", zip));
        } catch (Exception e) {
        	e.printStackTrace();
    ...
    @Produces("text/plain")
    @Path("/async/zip")
    @ApiOperation(value = ApiZIPAsync, notes = ApiZIPAsync + ". Toistaiseksi kirjeen malli on kiinteästi tiedostona jakelupaketissa. " + AsyncResponseLogicDocumentation)
    public Response asynczip(@ApiParam(value = "Muodostettavien jälkiohjauskirjeiden tiedot (1-n)", required = true) final JalkiohjauskirjeBatch input, @Context final HttpServletRequest request)
