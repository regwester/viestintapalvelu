...
public class EmailSender {
        ...
        if (message != null) { // message was created successfully
            ...
            if (EmailConstants.TEST_MODE.equals("NO")) {
                ...
                String logMsg = " in mailsending (Smtp: " + EmailConstants.SMTP + "), " + " SUBJECT '"
                        + emailMessage.getSubject() + "' FROM '" + emailMessage.getFrom() + "' REPLYTO '"
                        + (emailMessage.getReplyTo() == null ? "" : emailMessage.getReplyTo()) + "' TO '"
                        + emailAddress + "'";
                try {
    ...
    public MimeMessage createMessage(EmailMessage emailMessage, String emailAddress) throws MessagingException {
        ...
        mimeBodyPart.setContent(emailMessage.getBody(), (emailMessage.isHtml() ? "text/html" : "text/plain")
